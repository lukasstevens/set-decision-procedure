...
\title{Towards a Verified Tableau Prover for an Unquantified Fragment of Set Theory}
...
\begin{abstract}
  Using Isabelle/HOL, we verify the state-of-the-art decision procedure for multi-level syllogistic with singleton (\MLSS{} for short), which is an unquantified fragment of set theory.
  ...
  \keywords{First TODO \and Second TODO\todo{keywords}}
\end{abstract}
...
In particular, we consider an unquantified fragment which \citeauthor{new_fast_tableau}~\cite{new_fast_tableau} call multi-level syllogistic with singleton (\MLSS{}).
...
Since \MLSS{} admits a tableau calculus, we expect that the generation of certificates will be straightforward.
...
Here, the formalisation was instrumental because Isabelle immediately revealed broken proofs that needed fixing.
...
We overload this constant to return the set terms that are subterms of a set term, set atom, or formula, respectively. Lastly, we consider the subformulae of a formula and define a function \lstinline!subfms :: 'a fm $\Rightarrow$ 'a fm set! that computes them.
The functions \lstinline!subterms! and \lstinline!subfms! are defined in the usual way (cf.\ \appendixname~\ref{appendix:subterms}).
...
\begin{table*}[t]
  ...
  \end{tabular}
  \bigskip
  \caption{Linear expansion rules.\label{tab:lexpands} We omit the rules for $\interS$ and $\diffS$ as they coincide with the original paper (cf.\ \appendixname~\ref{appendix:rules}).}
\end{table*}
...
\begin{table*}[t]
  ...
  \end{tabular}
  \bigskip
  \caption{Branching expansion rules.\label{tab:bexpands} We write \lstinline!$\phi$! for \lstinline!last b! here. The omitted rules coincide with the original paper (cf.\ \appendixname~\ref{appendix:rules}).}
\end{table*}
...
\begin{proof}[Proof of Proposition~\ref{it:at_eq}]
  ...
  Considering that \lstinline!s $\in$ subterms' b! and the definition of \lstinline!realise!, we obtain a \lstinline!d! \lstinline!e = realise d! and \lstinline!d $\rightarrow_\text{bgraph b}$ s!.
...
\end{lstlisting}
\begin{proof}
  We only consider the case where \lstinline!&\fexpands{bs'}{b}&! was proved by applying the last rule to \lstinline!s $\neqS$ t! for some \lstinline!s! and \lstinline!t!.
  We have 
\begin{lstlisting}
...
By closely following a paper by \citeauthor{new_fast_tableau}~\cite{new_fast_tableau}, we developed a formalisation of a tableau calculus for an unquantified fragment of set theory called \MLSS{}.
...
\section{Expansion Rules\label{appendix:rules}}
...
\emph{Literals} are a type of subformulae of particular importance, so we define a predicate for them.
\begin{lstlisting}
