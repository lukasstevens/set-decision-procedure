...
Likewise, we fix the constant \lstinline!subterms :: 'b $\Rightarrow$ 'a pset_term set!.\todo{Explain b'}
We overload this constant to return the set terms that are subterms of a set term, set atom, or formula, respectively. Lastly, we compute the subformulas of a formula with the function \lstinline!subfms :: 'a fm $\Rightarrow$ 'a fm set!.
...
The HF sets are sufficient for our purposes as \MLSS{} is not powerful enough to force infinite models~\cite[Chapter~14.2]{set_theory}.\cite{Argue power: self-evident}
...
\end{lstlisting}
A tableau is called \emph{closed} if all of its branches are closed.\todo{What is a tableau?}
...
\begin{table*}[t]
  ...
  \caption{Linear expansion rules.\label[table]{tab:lexpands} We omit the rules for $\interS$ and $\diffS$ as they coincide with the original paper~\cite{new_fast_tableau} (cf.\ \appendixname~\ref{appendix:linear_rules}).}
  \begin{tabular}{ccc}
    \toprule
...
is saturated --- neither linear nor branching rules apply --- and open, but there clearly is no model for the initial formula \lstinline!&\fmAnd{\fmNeg{\fmNeg{\fmNeg{p}}}}{p}&!.\todo{Split into multiple sentences}
...
\begin{table*}[t]
  ...
  \caption{Branching expansion rules.\label[table]{tab:bexpands} We write \lstinline!$\phi$! for \lstinline!last b! here. The elided rules coincide with the original paper~\cite{new_fast_tableau} (cf.\ \appendixname~\ref{appendix:branching_rules}).}
...
Note that we use Hilbert's $\varepsilon$-operator in the form of \lstinline!SOME! to choose some rule that actually adds new formulas to the branch. 
...
To this end, we derive an upper bound for the number of distinct formulas in a branch whose proof we omit here for brevity (cf.\ \appendixname~\ref{appendix:bound}).
...
\end{lstlisting}
Finally, we lift this to formulas by using \lstinline!atoms! again.
\begin{lstlisting}
definition subterms :: 'a pset_fm $\Rightarrow$ 'a pset_term set where
  subterms $\phi$ $\equiv$ $\bigcup$(subterms ` atoms $\phi$)
\end{lstlisting}

We also consider the subformulas of a formula and define a function that computes them.
\begin{lstlisting}
...
The core of our argument is that the number of distinct formulas that can be derived with the expansion rules is finite.
