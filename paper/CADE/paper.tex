...
\begin{table*}[t]
  ...
  \caption{Linear expansion rules.\label[table]{tab:lexpands} We omit the rules for $\interS$ and $\diffS$ as they coincide with the original paper (cf.\ \appendixname~\ref{appendix:linear_rules}).}
  \begin{tabular}{ccc}
    \toprule
...
Furthermore, the term-for-term substitution \lstinline!l{s/t}! is restricted to replace \lstinline!t! only if it occurs as a top-level set term of \lstinline!l!.
...
In fact, we give an explicit upper bound for the number of formulae in a branch in \cref{sec:correct}.
...
\begin{table*}[t]
  ...
  \caption{Branching expansion rules.\label[table]{tab:bexpands} We write \lstinline!$\phi$! for \lstinline!last b! here. The elided rules coincide with the original paper (cf.\ \appendixname~\ref{appendix:branching_rules}).}
...
In \cref{sec:correct}, we will show that the domain predicate holds for the context that \lstinline!mlss_proc_branch! is called in.
...
To deal with this problem, we formalise a light-weight type system as displayed in \cref{fig:types}. 
