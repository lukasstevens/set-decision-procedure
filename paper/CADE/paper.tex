...
Both of these do not prove termination but there is a formalisation of a tableau calculus for the temporal logic CTL in Coq~\cite{ctl_tableau} that does.
...
Note that even branching rules are defined such that they never create new subterms, except for the last rule that adds a new variable to the branch.
...
The other concern is that Isabelle/HOL requires functions to be total, so a recursive function needs to terminate for it to be well-defined;
...
In \cref{sec:correct}, we will show that the domain predicate holds for the context in which the function \lstinline!mlss_proc_branch! is called in.
...
The purpose of the function is to determine whether we can expand a given branch to a closed tableau.
...
As soon as the branch is linearly saturated, we terminate if the branch is closed as the second equation shows.
...
Thus, we use \lstinline!mlss_proc_branch! is only on branches that result from applying the expansion rules.
We call this kind of branch \emph{well-formed}.
...
We use this notion in \cref{sec:correct} to state an upper bound for the cardinality of well-formed branches.
...
In the context of a well-formed branch, we can strengthen this characterisation to any set term and, in addition, we also get that there is no membership literal where a pure witness is on the right-hand side.
...
According to the definition of \lstinline!realise!, it would evaluate the pure witnesses would to the empty set \lstinline!0 :: hf!, were they not treated separately. 
...
\noindent We now lower the results on literals to set terms.
...
The final step for obtaining a proper model is to connect the realisation function to the semantics as defined in \cref{sec:semantics}.
...
A tableau calculus is sound if the corresponding formula is unsatisfiable for any closed tableau. 
We prove the following two properties to establish soundness:
\begin{enumerate}[label={(\arabic*)}]
  \item It is impossible to satisfy all formulas in a closed branch simultaneously. 
...
\end{lstlisting}
  ...
  We update \lstinline!M! to map \lstinline!x! to \lstinline!y! to obtain the assignment \lstinline!M'!.
...
To finish the proof of total correctness, note that every singleton branch is trivially well-formed; thus, termination, completeness, and soundness easily transfer to \lstinline!mlss_proc!.
\begin{lstlisting}
...
For this to work, we must map every branch expansion rule to a corresponding theorem in Isabelle/HOL.
...
\end{lstlisting}
in Isabelle/HOL.
By way of reification, we convert this to a formula of the shape
\begin{lstlisting}
 s' $\neqS$ t' $\fmAndSymbol$ s' $\inS$ A' $\unionS$ B' $\fmAndSymbol$ $\ldots$
\end{lstlisting}
in our set syntax for some \lstinline!s'!, \lstinline!t'!, \lstinline!A'!, and \lstinline!B'!.
When we apply the decision procedure to this formula, it might return a tableau proof that contains an application of the last branching rule to \lstinline!(s' $\neqS$ t') $\in$ set b!.
...
Note that the constructor $\emptyset$ now receives an additional argument indicating the level of each instance of $\emptyset$.
...
\begin{enumerate*}[label=(\arabic*)]
  \item The first equation of \lstinline!realise! now also must account for the urelements.
    ...
    This does not affect pure witnesses since they can not be related through equality atoms due to \reflemmatwo{}.
