...
The definition of \lstinline!subterms! and \lstinline!subfms! is standard so we omit them here and refer to the appendix.
...
we instead use the hierarchy of \textit{hereditarily finite sets} (HF sets) which fulfil all the same axioms as $\mathcal{V}$ except for the axiom of infinity.
...
\begin{lstlisting}
  &\Ist& :: ('a $\Rightarrow$ hf) $\Rightarrow$ 'a pset_term $\Rightarrow$ hf &\textnormal{and}&
  &\Isa& :: ('a $\Rightarrow$ hf) $\Rightarrow$ 'a pset_atom $\Rightarrow$ hf
\end{lstlisting}
...
\begin{lstlisting}
  e $\hfmem$ realise s $\land$ e $\hfnotmem$ realise t. 
\end{lstlisting}
...
\begin{lstlisting}[columns=fixed]
  realise u $\hfmem$ realise t = e $\hfmem$ realise t
\end{lstlisting}
...
\begin{lstlisting}
  &\Ist& M t$_\text{i}$ $\hfmem$ &\Ist& M t$_\text{i + 1 mod k + 1}$
\end{lstlisting}
...
\end{lstlisting}
  contains a cycle.
  This, however, is a contradiction because the membership relation of \lstinline!hf! is well-founded (see \autoref{sec:semantics} TODO) and well-foundedness is closed under transitive closure.
\end{proof}
...
\begin{lstlisting}
  &\phantom{$\lor$}& y $\hfmem$ &\Ist& M s $\land$ y $\hfnotmem$ &\Ist& M t
  $\lor$ y $\hfmem$ &\Ist& M t $\land$ y $\hfnotmem$ &\Ist& M s.
\end{lstlisting}
...
\begin{lstlisting}
  &\expandss{b'}{b}& $\land$ ($\exists$M. $\forall$$\psi$ $\in$ set b'. M $\models$ $\psi$) $\land$ bclosed b'
\end{lstlisting}
