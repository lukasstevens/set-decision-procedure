...
\begin{abstract}
  ...
  \keywords{First TODO \and Second TODO}
\end{abstract}
...
\begin{itemize}
  ...
  \item We restrict one branching rule TODO.
\end{itemize}
...
TODO: Mention SteP, Check reviewer comment.
...
The functions \lstinline!subterms! and \lstinline!subfms! are defined in the usual way (cf. Appendix TODO).
...
The implementation as displayed in TODO uses a couple of features of Isabelle/HOL's function package:
...
The proof of \hyperref[lst:lemma_2]{\lstinline!lemma_2!} is more technical than interesting so we refer the reader to the formalisation or paper TODO.
...
\begin{proof}
  ...
  Lastly, a membership cycle is impossible since the membership relation of \lstinline!hf! is well-founded (see \autoref{sec:semantics} TODO).
\end{proof}
...
\end{lstlisting}
\begin{proof}
  We proceed by strong induction on the difference between the upper bound due to \hyperref[lst:card_branch]{\lstinline!card_wf_branch_ub!} and \lstinline!|b|!.
...
\begin{lstlisting}
  $\llbracket$ &\expandss{b'}{b}&; set b $\subset$ set b' $\rrbracket$ $\Longrightarrow$ mlss_proc_branch_dom b'
\end{lstlisting}
for all \lstinline!b'!.
...
\noindent The above lemma allows us to use the computation induction rule of \lstinline!mlss_proc_branch! on well-formed branches, which we use to prove soundness and completeness.
