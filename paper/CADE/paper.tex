...
\begin{itemize}
  ...
  \item We restrict one branching rule\todo{Elaborate}.
\end{itemize}
...
\todo[inline]{Mention SteP, Check reviewr comment}
...
Ultimately, we conclude that the procedure must terminate because the number of formualae increases in each step but it is also bounded. 
\begin{lstlisting}
...
that corresponds to \lstinline!s' $\neqS$ t' $\fmAndSymbol$ s' $\inS$ A' $\unionS$ B' $\fmAndSymbol$ $\ldots$! in our set syntax.
Applying the decision procedure to this goal might result in a tableau proof containing an application of the last branching rule rule to \lstinline!(s' $\neqS$ t') $\in$ set b!, which creates the branches \lstinline![Var x $\inS$ s', Var x $\notinS$ t'] @ b! and \lstinline![Var x $\notinS$ s', Var x $\inS$ t'] @ b!.
But this can not be converted to a proof in Isabelle/HOL since \lstinline!s! and \lstinline!t! are not sets.

To deal with this problem, we introduce a light-weight type system that, given a mapping from variables to natural numbers, assigns a natural number to each set term. 

\begin{itemize}
  \item Give example from HOL
  \item Show how branch is expanded
  \item Mention implemented constraint solver
  \item Show how definition of realise needs to be changed
  \item Syntax needs to be changed for empty set
\end{itemize}
