...
\begin{lstlisting}
fun &\Isa& :: ('a $\Rightarrow$ V) $\Rightarrow$ 'a pset_atom $\Rightarrow$ bool where
...
| x $\in$ subterms' b
  $\Longrightarrow$ realise t = vset {realise ` parents (bgraph b) s}
...
\begin{lstlisting}[columns=fixed]
  realise u $\in$ elts (realise t) = e $\in$ elts (realise t)
\end{lstlisting}
...
\begin{lstlisting}
  ...
  $\leq$ 2 * card (subfms (last b)) + card {$\psi$ $\in$ set b. is_literal $\psi$}
  $\leq$ 2 * card (subfms (last b)) + 16 * (card (subterms (last b)))$^4$
\end{lstlisting}
...
\begin{lstlisting}
  ...
  $\leq$ card (subfms (last b)) + card ($\fmNegSymbol$ ` subfms (last b))
...
\begin{lstlisting}
  &\phantom{$\leq$}& card {$\psi$ $\in$ set b. is_literal $\psi$}
  $\leq$ 2 * card ($\bigcup$(atoms ` set b)) $\leq$ 2 * 2 * (card (subterms b))$^\text{2}$.
\end{lstlisting}
...
\begin{lstlisting}
  ...
  $\leq$ 4 * (card (subterms (last b)) + card (wits b))$^\text{2}$
  $\leq$ 4 * (card (subterms (last b)) + (card (subterms (last b))$^\text{2}$)$^\text{2}$
