...
This paper is about the formalisation in Isabelle/HOL of a tableau calculus for \MLSS{} due to \citeauthor{new_fast_tableau}~\cite{new_fast_tableau}\cite[Chapter 14]{set_theory}.
...
Abstract framework to prove completeness for first-order tableau systems~\cite{completeness_coinductive}.
Verified sequent calculus prover for first-order logic~\cite{sequent_first_order}.
Tableau calculus for temporal logic CTL in Coq~\cite{ctl_tableau}
Abstract framework for synthetic completeness in AFP~\cite{synthetic_completeness_afp}.
...
The functions \lstinline!subterms! and \lstinline!subfms! are defined in the usual way (cf. Appendix \todo{reference Appendix}).
...
we instead use the hierarchy of \textit{hereditarily finite sets} (HF sets) which fulfil all the same axioms as $\mathcal{V}$ --- that is, the axioms of ZF --- except for the axiom of infinity.\todo{Membership is well-founded.}
...
\todo[inline]{Introduce the initial formula}
...
Furthermore, the term-for-term substitution \lstinline!l{s/t}! is restricted to replace \lstinline!t! only if it occurs as a top-level set terms of \lstinline!l!, i.e.\ it occurs directly under one of the atom constructors \lstinline!$\inS$! or \lstinline!$\eqS$!.
...
\begin{table*}
  ...
  \end{tabular}
  \bigskip
  \caption{Branching expansion rules.\label{tab:bexpands} We write \lstinline!$\phi$! for \lstinline!last b! here.}
\end{table*}
...
The implementation as displayed in \todo{How to reference algorithm?} uses a couple of features of Isabelle/HOL's function package:
...
To this end, we derive an upper bound for the number of distinct formulae in a branch whose proof we omit here for brevity (cf. Appendix\todo{Reference Appendix}).
...
All in all, having the formalisation allows us to confidently modify the calculus as Isabelle immediately shows us where the proofs break.\todo{Longer description}
