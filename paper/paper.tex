...
\end{lstlisting}
The coherence property finishes the proof of completeness of the calculus since it gives us a model for every formula in an open and saturated branch.
...
lemma
  ...
  shows M $\models$ $\phi$
\end{lstlisting}
The same would also hold for branching rules if it were not for the last rule that adds additional variables.
Those variables need to be assigned specific values as we will see in the proof below.
\begin{lstlisting}[belowskip=0pt]
lemma
  assumes &\fexpandsW{bs'}{b}&
  assumes $\bigwedge$$\psi$. $\psi$ $\in$ set b $\Longrightarrow$ M $\models$ $\psi$
  shows $\exists$M'. $\exists$b' $\in$ bs'.
          $\forall$$\psi$ $\in$ set (b' @ b). M' $\models$ $\psi$
\end{lstlisting}
\begin{proof}
  Since the last branching rule is applied here, it holds that
\begin{lstlisting}
  bs' = {[Var x $\inS$ s, Var x $\notinS$ t],
         [Var x $\inS$ t, Var x $\notinS$ s]}
\end{lstlisting}
  for some fresh variable \lstinline!x!, i.e.\ \lstinline!x $\notin$ vars b! and in particular \lstinline!x $\notin$ vars s! as well as \lstinline!x $\notin$ vars t!.
...
\end{lstlisting}
  ...
  Together with the our assumption about \lstinline!y!, we infer that 
\end{proof}
...
\end{lstlisting}
\caption{Definition of the functions \lstinline|mlss_proc_branch| and \lstinline|mlss_proc|.}
\end{figure*}
