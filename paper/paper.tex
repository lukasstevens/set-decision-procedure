...
definition bgraph b $\equiv$
  let vs = Var ` wits' b $\cup$ subterms_branch' b
...
Observe that the vertices are partitioned into two sets, a collection of witnesses \lstinline!Var ` wits' b! and the rest of the set terms \lstinline!subterms_branch' b!, which are treated differently by the realization.
...
For the former set, we choose a function \lstinline!I! that assigns the elements \lstinline!x $\in$ Var ` wits' b! pairwise distinct sets with cardinality greater than that of the vertices.
...
Let \lstinline!b! be an open and saturated branch, i.e.\ it holds that \lstinline!sat b $\land$ bopen b!;
then, the following propositions hold:
\begin{itemize}
  \item We have \lstinline!realize s $=$ realize t! if \lstinline!s $\eqS$ t! is in \lstinline!b!.
  \item We have \lstinline!realize s $\neq$ realize t! if \lstinline!s $\neqS$ t! is in \lstinline!b!.
  \item We have \lstinline!realize s $\in$ elts (realize t)! if it holds that \lstinline!(s $\inS$ t)! is in \lstinline!b!. 
  \item We have \lstinline!realize s $\notin$ elts (realize t)! if it holds that \lstinline!(s $\notinS$ t)! is in \lstinline!b!. 
\end{itemize}

We extend this to the set term operators.
Let \lstinline!b! be an open and saturated branch.
\begin{itemize}
  \item If \lstinline!$\emptyset$ $\in$ subterms b!, then \lstinline!realize $\emptyset$ = 0!.
  \item Let \lstinline!$\star_\text{s}$ $\in$ {$\unionS$, $\diffS$, $\interS$}!. If \lstinline!s $\star_\text{s}$ t $\in$ subterms b!, then
    \begin{center}
