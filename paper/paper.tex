...
\begin{enumerate}[label=(\alph*)]
  \item\label{it:empty} It holds that \lstinline!realize $\emptyset$ = 0!.
...
\begin{proof}[Proof of Proposition~\ref{it:empty}]
  By definition, \lstinline!realize $\emptyset$! is equal to \lstinline!0! if \lstinline!$\emptyset$! is not a vertex of \lstinline!bgraph b!.
  ...
  Ultimately, we obtain \lstinline!realize $\emptyset$ = 0!.
\end{proof}
...
\begin{lstlisting}
  ($\lambda$x. realize (Var x)) $\models$ last b.
\end{lstlisting}
\end{proof}
...
\noindent To finish of the proof of total correctness, note that every singleton branch is trivially well-formed so termination, completeness, and soundless easily transfer to \lstinline!mlss_proc!.
\begin{lstlisting}
