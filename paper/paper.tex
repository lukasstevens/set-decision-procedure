...
\todo[inline]{Motivation: linear rules don't introduce subterms}
\begin{lstlisting}
fun subterms ::
...
Our model is defined in terms of the following three collections.
\begin{lstlisting}
...
\end{lstlisting}
The goal is to construct a syntactic model that is derived from the membership literals \lstinline!(s $\inS$ t) $\in$ set b!.
To this end, we construct a graph whose vertices are set terms and there is an edge from \lstinline!s! to \lstinline!t! in the graph if, and only if, \lstinline!(s $\inS$ t) $\in$ set b!.
...
definition bgraph b $\equiv$
  let vs = Var ` params' b $\cup$ subterms_branch' b
...
Observe that the vertices are partitioned into two sets, a collection of parameters \lstinline!Var ` params' b! and the rest of the set terms \lstinline!subterms_branch' b!, which are treated differently by the realization.
...
For the former set, we fix a function \lstinline!I! that assigns the elements \lstinline!x $\in$ Var ` params' b! pairwise distinct sets with cardinality greater than that of the vertices and then return the singleton set \lstinline!vset {I x}!.
The cardinality constraint ensures that realizations are distinct between those two sets.  
\begin{lstlisting}
abbreviation parents G s $\equiv$ {u. u $\rightarrow_\text{G}$ s}

function realize :: 'a pset_term $\Rightarrow$ V where
  x $\in$ Var ` params' b
  $\Longrightarrow$ realize x = vset {I x}
| x $\in$ subterms_branch' b
...
Together with the fact that \lstinline!bgraph b! is finite, we conclude that we can only recurse a finite number of times until we encounter a vertex that has no parents.
...
function mlss_proc_branch :: 'a branch $\Rightarrow$ bool where
  ...
  mlss_proc_branch = mlss_proc_branch ((SOME b'. &\lexpands{b'}{b}& $\land$ set b $\subset$ set (b' @ b)) @ b)
