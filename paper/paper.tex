...
For MLSS, we extend the notion of closedness with three additional rules; the first two are straightforward while the last one states that a branch is closed when the branch contains a membership cycle \lstinline!s $\inS$ t$_\text{1}$, t$_\text{1}$ $\inS$ t$_\text{2}$, $\ldots$, t$_\text{k}$ $\inS$ s!.
\begin{lstlisting}
...
\begin{itemize}
  \item Basic definitions and graph
  \item Graph is basically a syntactic model.
\end{itemize}
\begin{lstlisting}
